...
**Postman:**
- 导入: `specs/workshop/module-03-knowledge-copaw/openapi/kb-qa-contract.yaml`（相对仓库根）
- 功能: 高级 API 测试
- 优点: 支持自动化测试
...
**Dockerfile（示例：构建上下文为仓库根）**  
OpenAPI 契约已统一到 `specs/workshop/module-03-knowledge-copaw/openapi/`，与练习代码 `modules-practice/module-03/app/` 分离，复制路径需按 monorepo 调整，例如：
...
COPY modules-practice/module-03/app/ ./app/
COPY specs/workshop/module-03-knowledge-copaw/openapi/ ./openapi/
...
> 依赖列表以练习目录为准；若后续增加 `requirements.txt`，可改为 `COPY` + `pip install -r`。

**构建和运行（在仓库根执行）:**
```bash
docker build -f path/to/Dockerfile -t kb-qa-bff .
