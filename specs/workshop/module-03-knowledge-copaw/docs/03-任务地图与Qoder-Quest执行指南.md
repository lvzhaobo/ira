...
| 3 | 确认 **`02-TC`** 与冻结版一致 |
| 4 | 按 **§2 依赖顺序** 执行 Task；每 Task 打开 **本规格包** `docs/tasks/T-*.md`（仓库路径 `specs/workshop/module-03-knowledge-copaw/docs/tasks/`），将其中 **Quest 输入模板** 粘贴到 Qoder Quest（建议 **一 Task 一 Quest**，见 §4） |
...
| **每条 Quest 的输入** | 至少包含：**本规格包** `docs/tasks/T-*.md` 全文 + 本文件 **§3 统一约定**（或说明「已读仓库 AGENTS.md」） |

---

## 5. Tasks 文件存在哪里、格式是什么

| 路径（相对 `specs/workshop/module-03-knowledge-copaw/`） | 作用 |
|------|------|
| `docs/tasks/README.md` | **索引**：Task ID、对应 Spec、对应 TC、链接到各文件 |
| `docs/tasks/T-A.md` … `T-F.md` | **每 Task 一份**：依赖、Quest 输入模板、完成条件、与 TC 对齐 |

**契约与 Task 正文**以本规格包为真源；**示例实现与联调代码**在 `modules-practice/module-03/`。若用 Jira/飞书，可把 Task ID 与链接同步到看板，**正文仍以本仓库 Markdown 为准** 避免漂移。
