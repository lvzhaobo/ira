...
| 天 | 通过标准（至少满足 2/3） |
|----|-------------------------|
| D1 | [ ] 跑通 `ira` 前后端 [ ] 明确演示数据边界 [ ] 完成 1 张主流程草图 |
| D2 | [ ] 定位 1 条核心 API [ ] 写出 1 条 Given/When/Then [ ] 完成 1 次后端测试执行 |
| D3 | [ ] M3 Proposal/Spec/TC 口径一致 [ ] 跑通 1 条问答链路 [ ] 记录 1 条范围外决策 |
| D4 | [ ] 完成 M2 或 M4 的端到端演示 [ ] 说明降级策略 [ ] 完成 1 条模块对照结论 |
| D5 | [ ] 完成部署/运行演示 [ ] 输出问题清单与优先级 [ ] 完成课程复盘与后续计划 |
